...
//! seconds hand sweeps, and in the data color as the marks are. Each is
...
class RimNumerals {
    ...
    //! Air between the band and the digits, as a share of the ring so it
    ...
    //! The vector font a touch smaller than that system font. The system
    //! font is the smallest the watch has, so the fallback cannot follow.
    ...
    function prepare(dc as Dc, markReach as Number) as Void {
        ...
        for (var i = 0; i < _COUNT; i++) {
            var degrees = i * _HOURS_APART * Dial.DEGREES_PER_HOUR_MARK;
    ...
    //! Draw one numeral, in whatever color is set
    //! @param dc The drawing context
